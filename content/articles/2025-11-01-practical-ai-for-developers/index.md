...
But there was a massive friction point: **giving AI proper context**. When working in the browser, I'd copy code snippets and try to explain "this is the UserController" or "check the AuthService module"—but AI would be completely aimless and clueless. Class names and module names alone weren't enough context. Without file paths and line numbers, AI couldn't understand where the code lived or how pieces connected. I was wasting time manually typing out file paths and line numbers for every snippet.

So I built [copy_with_context.nvim](/articles/copy-with-context-nvim/), a Neovim plugin that automatically includes file paths and line numbers when copying code. Now when I share code with AI, it knows exactly where it is in the codebase. The experience went from "guess what file I'm talking about" to "here's precisely what I need help with at `src/controllers/user.js:42`". It's a small tool that solved a real workflow problem and made AI actually useful for understanding context.
